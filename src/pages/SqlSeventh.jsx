...
const SqlSeventh = ({ token }) => {
  ...
  return (
    <div>
      ...
      <div className="footer" style={{ width: '275px', height: '200px', backgroundColor: 'white', border: '2px solid black', position: 'absolute', bottom: '162.5px', display: 'flex', alignItems: 'center', justifyContent: 'center', left: 'calc(50% - 100px - 300px - 300px + 90px)' }}>
        <span className="footer-text" style={{ color: 'black', textAlign: 'center' }}>
          <p>
            The quote is inserted directly into
            the Sql string, and terminates the
            query early. This is what cause the
            syntax error we saw in the logs.
          </p>
          <button onClick={() => navigate('/SqlEigth')}>Next</button>
        </span>
      </div>
    </div>
  );
