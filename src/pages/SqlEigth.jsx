...
const SqlEigth = ({ token }) => {
  ...
  return (
    <div>
      ...
      <div className="footer" style={{ width: '250px', height: '200px', backgroundColor: 'white', border: '2px solid black', position: 'absolute', bottom: '162.5px', display: 'flex', alignItems: 'center', justifyContent: 'center', left: 'calc(50% - 100px - 300px - 300px + 100px)' }}>
        <span className="footer-text" style={{ color: 'black', textAlign: 'center' }}>
          <p>
            The behavior indicates that the
            application might be vulnerable
            to SQL injection.
          </p>
          <button onClick={() => navigate('/SqlNinth')}>Next</button>
        </span>
      </div>
    </div>
  );
