...
const SqlStart = ({ token }) => {
  let navigate = useNavigate();
  const [showErrorMessage, setShowErrorMessage] = useState(false); // Initially set to false
  ...
  function printLogMessages() {
    ...
    if (logsBody.innerHTML === '') {
      var initialMessage = "Application Initialized";
        ...
        <div className="application" style={{ width: '400px', height: '300px', padding: '20px', border: '1px solid #ccc', borderRadius: '5px', backgroundColor: '#fff', top: '30px' }}>
          ...
          <form id="login-form" action="#" method="post" onSubmit={handleSubmit} style={{ width: 'calc(100% - 20px)', position: 'relative' }}>
            <div style={{ position: 'relative' }}>
              <input type="text" id="username" name="username" placeholder="example@email.com" style={{ width: 'calc(100% - 20px)', padding: '10px', marginBottom: '10px', border: '1px solid #ccc', borderRadius: '5px' }} />
              {showErrorMessage && (
